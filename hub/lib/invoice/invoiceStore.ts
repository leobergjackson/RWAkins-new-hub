...
const KEY = (id: string) => `kubryx_inv_${id}`;
...
export const getInvoiceStats = () => {
  ...
  const totalUSDC = paid.reduce((s, i) => s + i.amountUSD, 0);
  const last = all[0]?.createdAt ?? null;
  return { totalCreated: all.length, totalPaid: paid.length, totalUSDC, lastCreated: last };
};
